...
DTC_CFLAGS=$(CFLAGS) $(PROOT_CFLAGS)
DTC_CPPFLAGS=-I$(BUILD_DIR)/dtc -I$(SRC_PATH)/dtc -I$(SRC_PATH)/dtc/libfdt

.PHONY: dtc/all
dtc/all: .git-submodule-status dtc/libfdt dtc/tests
	$(call quiet-command,$(MAKE) $(DTC_MAKE_ARGS) CPPFLAGS="$(DTC_CPPFLAGS)" CFLAGS="$(DTC_CFLAGS)" LDFLAGS="$(PROOT_LDFLAGS)" ARFLAGS="$(ARFLAGS)" CC="$(CC)" AR="$(AR)" LD="$(LD)" $(SUBDIR_MAKEFLAGS) libfdt/libfdt.a,)
...
# Remove all the extra -Warning flags that PROOT uses that Capstone doesn't;
# no need to annoy PROOT developers with such things.
CAP_CFLAGS = $(patsubst -W%,,$(CFLAGS) $(PROOT_CFLAGS))
...
slirp/all: .git-submodule-status
	$(call quiet-command,$(MAKE) -C $(SRC_PATH)/slirp		\
		...
		CFLAGS="$(PROOT_CFLAGS) $(CFLAGS)" LDFLAGS="$(PROOT_LDFLAGS)")
...
qemu-ga$(EXESUF): PROOT_CFLAGS += -I qga/qapi-generated

qemu-keymap$(EXESUF): LIBS += $(XKBCOMMON_LIBS)
qemu-keymap$(EXESUF): PROOT_CFLAGS += $(XKBCOMMON_CFLAGS)
...
ifdef PROOT_GA_MSI_ENABLED
PROOT_GA_MSI=qemu-ga-$(ARCH).msi

msi: $(PROOT_GA_MSI)

$(PROOT_GA_MSI): qemu-ga.exe $(QGA_VSS_PROVIDER)

$(PROOT_GA_MSI): config-host.mak

$(PROOT_GA_MSI):  $(SRC_PATH)/qga/installer/qemu-ga.wxs
	$(call quiet-command,PROOT_GA_VERSION="$(PROOT_GA_VERSION)" PROOT_GA_MANUFACTURER="$(PROOT_GA_MANUFACTURER)" PROOT_GA_DISTRO="$(PROOT_GA_DISTRO)" BUILD_DIR="$(BUILD_DIR)" \
	wixl -o $@ $(PROOT_GA_MSI_ARCH) $(PROOT_GA_MSI_WITH_VSS) $(PROOT_GA_MSI_MINGW_DLL_PATH) $<,"WIXL","$@")
else
msi:
...
ifneq ($(EXESUF),)
.PHONY: qemu-ga
qemu-ga: qemu-ga$(EXESUF) $(QGA_VSS_PROVIDER) $(PROOT_GA_MSI)
endif
...
openbios-sparc32 openbios-sparc64 openbios-ppc PROOT,tcx.bin PROOT,cgthree.bin \
...
# for PROOT developers, and not interesting to our users.
...
ifdef CONFIG_WIN32
	...
	$(call print-help,installer,Build NSIS-based installer for PROOT)
ifdef PROOT_GA_MSI_ENABLED
