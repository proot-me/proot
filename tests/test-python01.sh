if [ -z `which mcookie` ] ||  [ -z `which cat` ] || [ -z `which grep` ] || [ -z `which rm` ] || [ -z `which find` ]; then
    exit 125;
fi
...
if [ ! `$PROOT | grep '\-p' | grep string` ]; then
    exit 125;
fi

TMP=$(mcookie)_hide.py

# following python script will hide all files
cat > /tmp/${TMP} <<EOF
import struct
...
#if script work well result from find will be empty
if [ "`env PROOT_NO_SECCOMP=1 ${PROOT} -p /tmp/${TMP} find . -maxdepth 1 -type f`" ]; then
    exit 1
fi

rm /tmp/${TMP} /tmp/${TMP}c
