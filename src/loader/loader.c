...
void _start(void *cursor)
{
	...
	while (1) {
...
#if defined(OPEN)
			fd = SYSCALL(OPEN, 3, stmt->open.string_address,
				     O_RDONLY, 0);
#else
			fd = SYSCALL(OPENAT, 4, AT_FDCWD,
				     stmt->open.string_address, O_RDONLY,
				     0);
#endif
		...
		case LOAD_ACTION_MMAP_FILE:
			status =
			    SYSCALL(MMAP, 6, stmt->mmap.addr,
				    stmt->mmap.length, stmt->mmap.prot,
				    MAP_PRIVATE | MAP_FIXED, fd,
				    stmt->mmap.
				    offset >> MMAP_OFFSET_SHIFT);
			if (unlikely(status != stmt->mmap.addr))
				FATAL();

			if (stmt->mmap.clear_length != 0)
				clear(stmt->mmap.addr + stmt->mmap.length -
		...
		case LOAD_ACTION_MAKE_STACK_EXEC:
			...
			cursor +=
			    LOAD_STATEMENT_SIZE(*stmt, make_stack_exec);
			break;
		...
		case LOAD_ACTION_START:{
				word_t *cursor2 =
				    (word_t *) stmt->start.stack_pointer;
				...
				if (unlikely(traced))
					SYSCALL(EXECVE, 6, 1,
						...
						stmt->start.entry_point, 2,
						3, 4);
				else
					BRANCH(stmt->start.stack_pointer,
					       stmt->start.entry_point);
				FATAL();
			}
