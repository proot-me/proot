...
static int move_and_symlink_path(Tracee *tracee, Reg sysarg)
{
	...
	if (S_ISLNK(statl.st_mode)) {
		...
		strncpy(intermediate, original,
			strlen(original) - strlen(name));
			  ...
			  intptr_t data1, intptr_t data2 UNUSED)
{
	...
	case SYSCALL_ENTER_END:{
			...
			case PR_rename:
				...
				status =
				    decrement_link_count(tracee, SYSARG_2);
				if (status < 0)
					return status;
			...
			case PR_renameat:
				...
				status =
				    decrement_link_count(tracee, SYSARG_4);
				if (status < 0)
					return status;
			...
			case PR_unlink:
				...
				status =
				    decrement_link_count(tracee, SYSARG_1);
				if (status < 0)
					return status;
