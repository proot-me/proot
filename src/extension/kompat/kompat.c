 ...
 */
static bool modify_syscall(Tracee *tracee, const Config *config,
			   const Modif *modif)
{
	size_t i, j;
	word_t syscall;
	...
	syscall =
	    detranslate_sysnum(get_abi(tracee), modif->new_sysarg_num);
...
static void discard_fd_flags(Tracee *tracee, const Config *config,
			     int discarded_flags, int expected_release,
			     Reg sysarg)
{
	word_t flags;
...
static int handle_sysenter_end(Tracee *tracee, Config *config)
{
	...
	case PR_fchownat:{
			...
			modif.new_sysarg_num =
			    ((flags & AT_SYMLINK_NOFOLLOW) !=
			     0 ? PR_lchown : PR_chown);
	...
	case PR_fstatat64:{
			...
			if ((flags & AT_SYMLINK_NOFOLLOW) != 0)
				modif.new_sysarg_num =
				    (get_abi(tracee) !=
				     ABI_2 ? PR_lstat : PR_lstat64);
			else
				modif.new_sysarg_num =
				    (get_abi(tracee) !=
				     ABI_2 ? PR_stat : PR_stat64);
#else
			if ((flags & AT_SYMLINK_NOFOLLOW) != 0)
	...
	case PR_futex:{
			...
			if (!needs_kompat(config, KERNEL_VERSION(2, 6, 22))
	...
	case PR_openat:{
			...
			modified = modify_syscall(tracee, config, &modif);
			discard_fd_flags(tracee, config, O_CLOEXEC,
	...
	case PR_open:
		discard_fd_flags(tracee, config, O_CLOEXEC,
				 KERNEL_VERSION(2, 6, 23), SYSARG_2);
		return 0;
...
static void adjust_elf_auxv(Tracee *tracee, Config *config)
{
	...
	reserve_size =
	    ((reserve_size - 1) / STACK_ALIGNMENT + 1) * STACK_ALIGNMENT;
 ...
 */
static void emulate_fd_flags(Tracee *tracee, word_t fd, Reg sysarg,
			     int emulated_flags)
{
	word_t flags;
	...
	if ((emulated_flags & flags & O_CLOEXEC) != 0)
		register_chained_syscall(tracee, PR_fcntl, fd, F_SETFD,
					 FD_CLOEXEC, 0, 0, 0);

	if ((emulated_flags & flags & O_NONBLOCK) != 0)
		register_chained_syscall(tracee, PR_fcntl, fd, F_SETFL,
					 O_NONBLOCK, 0, 0, 0);

	force_chain_final_result(tracee,
...
static int handle_sysexit_end(Tracee *tracee, Config *config)
{
	...
	case PR_uname:{
			 ...
			 * since < 2.6.0.  */
			status =
			    write_data(tracee, address, &config->utsname,
				       sizeof(config->utsname));
			if (status < 0)
				return status;
			return 0;
		}
	...
	case PR_sethostname:{
			...
			if (length >
			    sizeof(config->utsname.domainname) - 1)
				return -EINVAL;

			/* Because of the test above.  */
			assert(sizeof(config->utsname.domainname) ==
			       sizeof(config->utsname.nodename));
	...
	case PR_accept4:
		if (get_sysnum(tracee, MODIFIED) == PR_accept)
			emulate_fd_flags(tracee, result, SYSARG_4,
					 O_CLOEXEC | O_NONBLOCK);
		return 0;

	case PR_dup3:
		if (get_sysnum(tracee, MODIFIED) == PR_dup2)
			emulate_fd_flags(tracee,
	...
	case PR_epoll_create1:
		if (get_sysnum(tracee, MODIFIED) == PR_epoll_create)
			emulate_fd_flags(tracee, result, SYSARG_1,
					 O_CLOEXEC | O_NONBLOCK);
		return 0;

	case PR_eventfd2:
		if (get_sysnum(tracee, MODIFIED) == PR_eventfd)
			emulate_fd_flags(tracee, result, SYSARG_2,
					 O_CLOEXEC | O_NONBLOCK);
		return 0;
	...
	case PR_fcntl:{
			...
			register_chained_syscall(tracee, PR_fcntl, result,
						 F_SETFD, FD_CLOEXEC, 0, 0,
						 0);
			force_chain_final_result(tracee,
	...
	case PR_openat:
		if (needs_kompat(config, KERNEL_VERSION(2, 6, 23)))
			emulate_fd_flags(tracee, result, SYSARG_3,
					 O_CLOEXEC);
		return 0;
	...
	case PR_pipe2:{
			...
			status =
			    read_data(tracee, fds,
				      peek_reg(tracee, MODIFIED, SYSARG_1),
				      sizeof(fds));
			if (status < 0)
				return 0;

			emulate_fd_flags(tracee, fds[0], SYSARG_2,
					 O_CLOEXEC | O_NONBLOCK);
			emulate_fd_flags(tracee, fds[1], SYSARG_2,
					 O_CLOEXEC | O_NONBLOCK);
	...
	case PR_socketpair:{
			...
			if (!needs_kompat
			    (config, KERNEL_VERSION(2, 6, 27)))
				return 0;

			status =
			    read_data(tracee, fds,
				      peek_reg(tracee, MODIFIED, SYSARG_4),
				      sizeof(fds));
			if (status < 0)
				return 0;
...
static int parse_utsname(Config *config, const char *string)
{
	...
	if (status < 0 || getenv("PROOT_FORCE_KOMPAT") != NULL)
		...
		config->actual_release =
		    parse_kernel_release(utsname.release);
...
#undef PARSE
		...
		if (errno != 0 || end2[0] != '\\') {
			note(NULL, ERROR, USER,
			     "can't find hwcap field in '%s'", string);
			return -1;
		}
	} else {
		size_t length;

		memcpy(&config->utsname, &utsname,
		       sizeof(config->utsname));

		length =
		    MIN(strlen(string),
			sizeof(config->utsname.release) - 1);
	...
	config->virtual_release =
	    parse_kernel_release(config->utsname.release);
		    ...
		    intptr_t data1, intptr_t data2 UNUSED)
{
	...
	case INITIALIZATION:{
			...
			status =
			    parse_utsname(config, (const char *) data1);
			if (status < 0)
				return -1;
