...
extern PyObject *PyInit__proot(void);
...
void init_python_env()
{
	...
	if (status < 0 || status >= PATH_MAX) {
	    ...
	    /* Register "_proot" as a built-in module *before*
	     * initializing the interpreter, so that the
	     * "import _proot" done by proot.py (generated by swig)
	     * actually resolves to it.  Calling PyInit__proot()
	     * directly, as done before, only builds a module object
	     * and throws it away: it never reaches sys.modules.  */
#if PY_VERSION_HEX >= 0x03000000
	    PyImport_AppendInittab("_proot", PyInit__proot);
#else
	    PyImport_AppendInittab("_proot", init_proot);
#endif
	    Py_Initialize();
					...
					intptr_t data1, intptr_t data2)
{
    ...
    /* init_python_env() may have failed to load python_extension.py
     * or to find python_callback() in it; don't call through a NULL
     * pointer in that case.  */
    if (python_callback_func == NULL)
	return 0;
