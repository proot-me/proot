...
extern int which(Tracee * tracee, const char *paths,
		 char host_path[PATH_MAX], const char *command);
extern int realpath2(Tracee * tracee, char host_path[PATH_MAX],
		     const char *path, bool deref_final);
...
extern int translate_path(Tracee * tracee, char host_path[PATH_MAX],
			  int dir_fd, const char *guest_path,
			  bool deref_final);
