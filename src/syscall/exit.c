...
void translate_syscall_exit(Tracee *tracee)
{
	...
	case PR_getcwd:{
			...
			size =
			    (size_t) peek_reg(tracee, ORIGINAL, SYSARG_2);
			if (size == 0) {
				status = -EINVAL;
				break;
			}
			...
			status =
			    translate_path(tracee, path, AT_FDCWD, ".",
					   false);
			if (status < 0)
				break;
			...
			status =
			    write_data(tracee, output, tracee->fs->cwd,
				       new_size);
			if (status < 0)
				break;
	...
	case PR_getpeername:{
			...
			status =
			    translate_socketcall_exit(tracee, sock_addr,
						      size_addr, max_size);
			if (status < 0)
				break;
...
#undef POKE_WORD
	...
	case PR_renameat2:{
			...
			status =
			    read_path(tracee, old_path,
				      peek_reg(tracee, MODIFIED, old_reg));
			if (status < 0)
				break;
			...
			old_length =
			    (status >
			     0 ? status - 1 : (ssize_t) strlen(old_path));

			/* Nothing special to do if the moved path is not the
			 * current working directory.  */
			comparison =
			    compare_paths(old_path, tracee->fs->cwd);
			 ...
			 * "point-of-view" as tracee->fs->cwd (guest).  */
			status =
			    read_path(tracee, new_path,
				      peek_reg(tracee, MODIFIED, new_reg));
			if (status < 0)
				break;
			...
			new_length =
			    (status >
			     0 ? status - 1 : (ssize_t) strlen(new_path));
			...
			substitute_path_prefix(old_path, old_length,
					       new_path, new_length);
	...
	case PR_readlinkat:{
			 ...
			 * readlink(2).  */
			status =
			    read_data(tracee, referee, output, old_size);
			if (status < 0)
				break;
			referee[old_size] = '\0';
			...
			status =
			    detranslate_path(tracee, referee, referer);
			if (status < 0)
				break;
			...
			if ((size_t) status < max_size) {
				new_size = status - 1;
				status =
				    write_data(tracee, output, referee,
					       status);
			} else {
				new_size = max_size;
				status =
	...
	case PR_uname:{
			...
			status =
			    read_data(tracee, &utsname, address,
				      sizeof(utsname));
			if (status < 0)
				break;
			...
			status =
			    write_data(tracee, address, &utsname,
				       sizeof(utsname));
			if (status < 0)
				break;
	...
	case PR_prlimit64:
		...
		status =
		    translate_setrlimit_exit(tracee,
					     syscall_number ==
					     PR_prlimit64);
		if (status < 0)
			break;
