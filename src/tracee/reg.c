 ...
 */
void print_current_regs(Tracee *tracee, int verbose_level,
			const char *message)
{
	...
	note(tracee, INFO, INTERNAL,
	     "vpid %" PRIu64
	     ": %s: %s(0x%lx, 0x%lx, 0x%lx, 0x%lx, 0x%lx, 0x%lx) = 0x%lx [0x%lx, %d]",
	     tracee->vpid, message,
	     stringify_sysnum(get_sysnum(tracee, CURRENT)),
	     peek_reg(tracee, CURRENT, SYSARG_1), peek_reg(tracee, CURRENT,
							   SYSARG_2),
	     peek_reg(tracee, CURRENT, SYSARG_3), peek_reg(tracee, CURRENT,
							   SYSARG_4),
	     peek_reg(tracee, CURRENT, SYSARG_5), peek_reg(tracee, CURRENT,
							   SYSARG_6),
	     peek_reg(tracee, CURRENT, SYSARG_RESULT), peek_reg(tracee,
								CURRENT,
								STACK_POINTER),
	     get_abi(tracee));
}
...
void save_current_regs(Tracee *tracee, RegVersion version)
{
	...
	memcpy(&tracee->_regs[version], &tracee->_regs[CURRENT],
...
#else
	status =
	    ptrace(PTRACE_GETREGS, tracee->pid, NULL,
...
#define	 	RESTORE(sysarg) (void) (reg_offset[SYSARG_RESULT] != reg_offset[sysarg] && \
		...
		if (current_sysnum != REG(tracee, ORIGINAL, SYSARG_NUM)) {
			...
			status =
			    ptrace(PTRACE_SETREGSET, tracee->pid,
				   NT_ARM_SYSTEM_CALL, &regs);
			if (status < 0)
				note(tracee, WARNING, SYSTEM,
				     "can't set the syscall number");
		}
		...
		status =
		    ptrace(PTRACE_SETREGSET, tracee->pid, NT_PRSTATUS,
			   &regs);
#else
		...
		if (current_sysnum != REG(tracee, ORIGINAL, SYSARG_NUM)) {
			status =
			    ptrace(PTRACE_SET_SYSCALL, tracee->pid, 0,
				   current_sysnum);
			if (status < 0)
				note(tracee, WARNING, SYSTEM,
				     "can't set the syscall number");
		...
		status =
		    ptrace(PTRACE_SETREGS, tracee->pid, NULL,
			   &tracee->_regs[CURRENT]);
#endif
		if (status < 0)
			return status;
